...
class POSTokenizer(object):
    ...
    def __cut_DAG_NO_HMM(self, sentence):
        """不带HMM模型的切分，调用__cut_detail"""
        DAG = self.tokenizer.get_DAG(sentence)  # 构建词图
        route = {}
        self.tokenizer.calc(sentence, DAG, route)  # 计算最优路径
        x = 0
        N = len(sentence)
        buf = ''
        while x < N:
            ...
            if re_eng1.match(l_word):
                ...
                if buf:
                    yield pair(buf, 'eng')
                    buf = ''
                yield pair(l_word, self.word_tag_tab.get(l_word, 'x'))  # 从词库中获取词性
                x = y
        if buf:
            yield pair(buf, 'eng')
            buf = ''
        ...
        while x < N:
            ...
            if y - x == 1:
                ...
                if buf:
                    ...
                    elif not self.tokenizer.FREQ.get(buf):  # 连续单字，走HMM词性模型
                        recognized = self.__cut_detail(buf)
                        for t in recognized:  # 识别出新词及其词性
                            yield t
                    else:
                        for elem in buf:
